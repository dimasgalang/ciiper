...
<script>
    ...
    $('.btn-void-record').on('click', function () {
            ...
            $("#modal-text-record-void").text('Apakah anda yakin ingin menghapus Market ' + $(this).data('void-name') + '?');
    });
    $('.btn-restore-record').on('click', function () {
            $('#btn-confirm-restore').attr('href', $(this).data('restore-link'));
            $("#modal-text-record-restore").text('Apakah anda yakin ingin mengembalikan Market ' + $(this).data('restore-name') + '?');
    });
</script>
</html>